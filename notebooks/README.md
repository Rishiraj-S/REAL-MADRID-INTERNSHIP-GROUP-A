...
are independent exploration/tuning notebooks that all depend on its output.

The canonical deployable model artifacts are generated by `train_models.py`, not
by notebook pickle exports. The notebooks document exploration and tuning choices;
the app expects `model.json`, `feature_cols.pkl`, and `transform.pkl` under
`models/xgboost/{target}/`.

Notebook diagnostic plots should be saved under `reports/figures/{target}/`.
Notebook-only sklearn pipeline pickles, if regenerated, should stay under
`models/notebook_experiments/{target}/` and are not used by the app.
...
| 8. Model Persistence | Historical notebook persistence; use `train_models.py` for deployable artifacts |
...
| B.9 Model Persistence | Historical notebook persistence; use `train_models.py` for deployable artifacts |
