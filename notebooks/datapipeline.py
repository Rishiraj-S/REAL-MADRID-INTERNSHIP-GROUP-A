"""
datapipeline.py — Shared preprocessing pipeline for total_distance,
accelerations, and sprint_distance models.

Exports
-------
run_pipeline(target)   → dict with all training-ready objects
prepare_test(...)      → apply train transforms to the held-out test frame
_add_features / _encode_dow  (used internally and by prepare_test)
"""
...
def treat_outliers(df: pd.DataFrame, target: str) -> pd.DataFrame:
    """IQR-cap total_distance (always) and the target (if different)."""
...
def aggregate_daily(df: pd.DataFrame, target: str) -> tuple:
    ...
    # Build agg dict — always include target; include total_distance as covariate
    # only when it isn't the target itself (avoids duplicate key error)
    agg_dict = {target: (target, 'sum')}
    if target != 'total_distance':
...
def spine_fill(daily: pd.DataFrame, target: str, HAS_COLS: list) -> pd.DataFrame:
    """Insert rest-day rows so each player has a contiguous date range."""
    zero_cols = [target]
    ...
    spines = [
        ...
        for pid, row in tqdm(
            player_date_range.iterrows(), total=len(player_date_range), desc="Building date spine"
        )
    ]
...
def prepare_test(test_df: pd.DataFrame, target: str,
                 scaler, feature_cols: list) -> pd.DataFrame:
...
def run_pipeline(target: str, test_size: float = 0.2,
    ...
    Parameters
    ----------
    target       : 'total_distance' | 'accelerations' | 'sprint_distance'
