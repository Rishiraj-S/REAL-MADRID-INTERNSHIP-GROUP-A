...
from real_madrid_acwr.modeling.acceleration_model_train import main as train_accelerations
from real_madrid_acwr.modeling.sprint_distance_model_train import main as train_sprint_distance
from real_madrid_acwr.modeling.total_distance_model_train import main as train_total_distance


def main() -> None:
    # Build full daily frame (all 3 metrics) once for the app
    print("Building daily.parquet for app inference…")
    df    = load_data()
    df    = clean_data(df)
    # IQR-cap total_distance only (safe to apply before target-specific training)
