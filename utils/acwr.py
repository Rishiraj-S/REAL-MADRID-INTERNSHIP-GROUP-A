...
def compute_acwr(
    daily_loads: np.ndarray | pd.Series,
    ...
    if isinstance(daily_loads, np.ndarray):
    ...
    acwr = pd.Series(acwr, index=loads.index)
...
def compute_acwr_with_forecast(
    historical_loads: np.ndarray | pd.Series,
    forecast_loads: np.ndarray | pd.Series,
    acute_window: int = ACUTE_WINDOW,
    chronic_window: int = CHRONIC_WINDOW,
    warmup_days: int | None = None,
) -> pd.DataFrame:
